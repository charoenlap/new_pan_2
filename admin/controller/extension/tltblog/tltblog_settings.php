...
define('TLT_BLOG_VERSION', '5.1.8.2');
...
class ControllerExtensionTltBlogTltBlogSettings extends Controller {
    ...
    public function install()
    {
        ...
        if (!$check_tables) {
            ...
            if (!$this->model_extension_tltblog_settings->is30()) {
                $this->model_extension_tltblog_settings->upgradeFrom2();
                ...
                foreach ($languages as $language) {
                    $path[$language['language_id']] = $language['code'] == $this->config->get('config_language') ? $this->config->get('tltblog_path') : '';
                }

                $tltblog_settings['tltblog_path'] = $path;
            }

            if ($this->config->has('tltblog_db_version') && $db_version !== TLT_BLOG_DB_VERSION) {
                $this->model_extension_tltblog_settings->upgrade();
            }

            if (empty($tltblog_settings)) {
                $tltblog_settings = $this->model_extension_tltblog_settings->getSettings();
            }
        }
