...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		if (isset($this->request->post['free'])) {
			$data['free'] = $this->request->post['free'];
		} elseif (!empty($product_info)) {
			$data['free'] = $product_info['free'];
		} else {
			$data['free'] = 0;
		}
		...
		if (isset($this->request->post['product_free'])) {
			$products = $this->request->post['product_free'];
		} elseif (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getProductFree($this->request->get['product_id']);
		} else {
			$products = array();
		}
	...
	protected function validateForm() {
		...
		if (empty($this->request->post['length'])||$this->request->post['length']<=0) {
			$this->error['length'] = 'กรุณาใส่ขนาดความกว้าง';
		}

		if (empty($this->request->post['width'])||$this->request->post['width']<=0) {
			$this->error['width'] = 'กรุณาใส่ขนาดความยาว';
		}

		if (empty($this->request->post['height'])||$this->request->post['height']<=0) {
			$this->error['height'] = 'กรุณาใส่ขนาดความสูง';
		}
		...
		if ($this->error && !isset($this->error['warning'])) {
			$this->error['warning'] = $this->language->get('error_warning').json_encode($this->error);
		}
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			if (isset($this->request->get['filter_free'])) {
				$filter_free = (int)$this->request->get['filter_free'];
			} else {
				$filter_free = 0;
			}
