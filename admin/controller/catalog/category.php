...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_category_id'])) {
			$filter_data['filter_category_id'] = $filter_category_id;
			$temp = $this->model_catalog_category->getCategories($filter_data);
			$category_total = count($temp);
		}
		...
		foreach ($results as $result) {
			if ( $result['parent_id'] == 0 ) {
				...
				// if ($result['category_id']==241) {
				// 	$subs = $this->model_catalog_category->getCategories($result['category_id']);
				// 	foreach ($subs as $sub) {
				// 		$data['filter_category'][] = array(
				// 			'name' => $sub['name'],
	...
	protected function getForm() {
		...
		if (isset($this->request->get['filter_category_id']) && $this->request->get['filter_category_id'] > 0) {
			$parent_info = $this->model_catalog_category->getCategory($this->request->get['filter_category_id']);
		}
		...
		} elseif (isset($this->request->get['filter_category_id']) && $this->request->get['filter_category_id'] > 0 ) {
		...
		if (isset($this->request->post['parent_id'])) {
			...
			$data['parent_id'] = (int)$this->request->get['filter_category_id'];
		} else {
			$data['parent_id'] = 0;
		}
