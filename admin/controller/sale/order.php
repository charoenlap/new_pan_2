...
class ControllerSaleOrder extends Controller {
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			foreach ($products as $product) {
				...
				$products_free = $this->model_sale_order->getProductFree($product['product_id']);
				if (count($products_free)>0) {
					foreach ($products_free as $pro) {
						$profree_info = $this->model_catalog_product->getProduct($pro['product_id']);
						$data['order_products'][] = array(
							'product_id' => $profree_info['product_id'],
							'name'       => $profree_info['name'],
							'model'      => $profree_info['model'],
							'option'     => '',
							'quantity'   => $profree_info['quantity'],
							'price'      => $profree_info['price'],
							'total'      => $profree_info['total'],
							'reward'     => $profree_info['reward']
						);
					}
				}
			}
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['shipping_method'] = $order_info['shipping_method'];
			$data['payment_method'] = $order_info['payment_method'];
			$data['payment_taxid']	= $order_info['payment_taxid'];

			// Payment Address
			if ($order_info['payment_address_format']) {
				$format = $order_info['payment_address_format'];
			} else {
				$format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}' . "\n" ."เลขประจำตัวผู้เสียภาษี :". ' {payment_taxid}';
			}
			...
			$replace = array(
				...
				'payment_taxid'	=> $order_info['payment_taxid'],
			);
			...
			foreach ($products as $product) {
				...
				foreach ($products_free as $pro) {
					...
					$data['products'][] = array(
						'product_id' => $profree_info['product_id'],
						'name'       => $profree_info['name'].' (สินค้าแถมฟรี)',
						'model'      => $profree_info['model'],
						'option'     => '',
						'quantity'   => 1,
						'price'      => 0,
						'total'      => 0,
						'href'       => $this->url->link('catalog/product/edit', 'user_token=' . $this->session->data['user_token'] . '&product_id=' . $profree_info['product_id'], true)
					);
				}
			}
			...
			$data['recommend'] = $order_info['recommend'];
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				if ($order_info['shipping_country_id']!=209) {
					$data['title']                = 'Invoice';
					// Text
					$data['text_success']               = 'Success: You have modified orders!';
					$data['text_list']                  = 'Order List';
					$data['text_add']                   = 'Add Order';
					$data['text_edit']                  = 'Edit Order';
					$data['text_filter']                = 'Filter';
					$data['text_order_detail']          = 'Order Details';
					$data['text_customer_detail']       = 'Customer Details';
					$data['text_option']                = 'Options';
					$data['text_store']                 = 'Store';
					$data['text_date_added']            = 'Date Added';
					$data['text_payment_method']        = 'Payment Method';
					$data['text_shipping_method']       = 'Shipping Method';
					$data['text_customer']              = 'Customer';
					$data['text_customer_group']        = 'Customer Group';
					$data['text_email']                 = 'E-Mail';
					$data['text_telephone']             = 'Telephone';
					$data['text_fax']                   = 'Fax';
					$data['text_invoice']               = 'Invoice';
					$data['text_reward']                = 'Reward Points';
					$data['text_affiliate']             = 'Affiliate';
					$data['text_order']                 = 'Order (#%s)';
					$data['text_payment_address']       = 'Payment Address : รายละเอียดใบกำกับภาษี';
					$data['text_shipping_address']      = 'Shipping Address : ที่อยู่ผู้รับสินค้า';
					$data['text_comment']               = 'Customer Comment';
					$data['text_history']               = 'Order History';
					$data['text_history_add']           = 'Add Order History';
					$data['text_account_custom_field']  = 'Account Custom Fields';
					$data['text_payment_custom_field']  = 'Payment Address Custom Fields';
					$data['text_shipping_custom_field'] = 'Shipping Address Custom Fields';
					$data['text_browser']               = 'Browser';
					$data['text_ip']                    = 'IP Address';
					$data['text_forwarded_ip']          = 'Forwarded IP';
					$data['text_user_agent']            = 'User Agent';
					$data['text_accept_language']       = 'Accept Language';
					...
					$data['text_product']               = 'Add Product(s)';
					$data['text_voucher']               = 'Add Voucher(s)';
					$data['text_shipping']              = 'Shipping';
					$data['text_contact']               = 'Contact';
					$data['text_reward_added']          = 'Success: Reward points added!';
					$data['text_reward_removed']        = 'Success: Reward points removed!';
					$data['text_commission_added']      = 'Success: Commission added!';
					$data['text_commission_removed']    = 'Success: Commission removed!';
					$data['text_restock']               = 'Success: Products have been restocked!';
					$data['text_upload']                = 'Your file was successfully uploaded!';
					$data['text_picklist']              = 'Dispatch Note';

					// Column
					$data['column_order_id']            = 'Order ID';
					$data['column_customer']            = 'Customer';
					$data['column_status']              = 'Status';
					$data['column_date_added']          = 'Date Added';
					$data['column_date_modified']       = 'Date Modified';
					...
					$data['column_notify']              = 'Customer Notified';
					$data['column_location']            = 'Location';
					$data['column_reference']           = 'Reference';
					$data['column_action']              = 'Action';
					$data['column_weight']              = 'Product Weight';

					// Entry
					$data['entry_store']                = 'Store';
					$data['entry_customer']             = 'Customer';
					...
					$data['entry_address']              = 'Choose Address';
					$data['entry_company']              = 'Company';
					$data['entry_address_1']            = 'Address 1';
					...
					$data['entry_zone_code']            = 'Region / State Code';
					$data['entry_product']              = 'Choose Product';
					$data['entry_option']               = 'Choose Option(s)';
					$data['entry_quantity']             = 'Quantity';
					$data['entry_to_name']              = 'Recipient\'s Name';
					$data['entry_to_email']             = 'Recipient\'s E-mail';
					$data['entry_from_name']            = 'Sender\'s Name';
					$data['entry_from_email']           = 'Sender\'s E-mail';
					$data['entry_theme']                = 'Gift Certificate Theme';
					$data['entry_message']              = 'Message';
					$data['entry_amount']               = 'Amount';
					$data['entry_affiliate']            = 'Affiliate';
					$data['entry_order_status']         = 'Order Status';
					$data['entry_notify']               = 'Notify Customer';
					$data['entry_override']             = 'Override';
					$data['entry_comment']              = 'Comment';
					$data['entry_currency']             = 'Currency';
					$data['entry_shipping_method']      = 'Shipping Method';
					$data['entry_payment_method']       = 'Payment Method';
					$data['entry_coupon']               = 'Coupon';
					$data['entry_voucher']              = 'Voucher';
					$data['entry_reward']               = 'Reward';
					$data['entry_order_id']             = 'Order ID';
					$data['entry_total']                = 'Total';
					$data['entry_date_added']           = 'Date Added';
					$data['entry_date_modified']        = 'Date Modified';

					// Help
					$data['help_override']              = 'If the customers order is being blocked from changing the order status due to an anti-fraud extension enable override.';

					// Error
					$data['error_warning']              = 'Warning: Please check the form carefully for errors!';
					$data['error_permission']           = 'Warning: You do not have permission to modify orders!';
					$data['error_action']               = 'Warning: Could not complete this action!';
					$data['error_filetype']             = 'Invalid file type!';
				}
	...
	public function shipping() {
		...
		foreach ($orders as $order_id) {
			$order_info = $this->model_sale_order->getOrder($order_id);
			// var_dump($order_info['shipping_code']);
			// echo '<pre>';
			// print_r($order_info);
			// echo '</pre>';
