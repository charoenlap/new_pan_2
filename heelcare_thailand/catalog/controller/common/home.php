...
class ControllerCommonHome extends Controller {
	public function index() {
		$this->document->setTitle('Heelcare');
		...
		$results = $this->model_catalog_category->getCategories(96);
		foreach ($results as $result) {
		...
		if (count($results) < 10 && isset($temp_product_id)) {
			$filter = array('filter_not_product_id'=>$temp_product_id);
		...
		$data['firsthalf2'] = array_slice($data['product_popular'], 0, (count($data['product_popular']) / 2));
		$data['secondhalf2'] = array_slice($data['product_popular'], (count($data['product_popular']) / 2)-1);
