...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$this->log->write("Add Order Option #" . $order_product_id);

				foreach ($product['option'] as $option) {
					$sqloption = "INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', `name` = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'";
					$this->log->write("Add Order Option #" . $sqloption);
					$this->db->query($sqloption);
				}
			}
		}
	...
	public function getOrders($filter = array()) {
		$sql = "SELECT * FROM " . DB_PREFIX . "order WHERE order_status_id = '1' AND customer_id = 8 ";

		if (isset($filter['max_date'])) {
			$sql .= " AND date_modified < '" . $filter['max_date'] . "'";
		}
		...
		foreach ($query->rows as $result) {
			$orders[] = $this->getOrder($result['order_id']);
		}
	...
	public function getProductFree($product_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_free WHERE free_id = '" . (int)$product_id . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			// $mailphpmailer = new MailPHPMailer();
			// $mailphpmailer->debug = 0;
			// $mailphpmailer->port = 465;
			// $mailphpmailer->protocal = 'ssl';
			// $mailphpmailer->host = 'ssl://smtp.gmail.com';
			// $mailphpmailer->username = $this->config->get('config_mail_smtp_username');
			// $mailphpmailer->password = html_entity_decode($this->config->get('config_mail_smtp_password'), ENT_QUOTES, 'UTF-8');

			// // who sent?
			// $mailphpmailer->form = $this->config->get('config_email');
			// $mailphpmailer->sender = html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8');
			// $mailphpmailer->reply = $this->config->get('config_email');

			// $mailphpmailer->to = $this->config->get('config_email');
			// $mailphpmailer->recipient = html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8');

			// $mailphpmailer->subject = html_entity_decode('Order '.$order_info['order_id']);
			// $mailphpmailer->html = html_entity_decode($this->load->view('mail/order_edit', $data));
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_products as $order_product) {
					...
					// Free Product
					$product_frees = $this->getProductFree($order_product['product_id']);
					foreach ($product_frees as $product_free) {
						$this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity - " . (int)$order_product['quantity'] . ") WHERE product_id = '" . (int)$product_free['product_id'] . "' AND subtract = '1'");
					}
				}
