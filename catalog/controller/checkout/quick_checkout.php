...
class ControllerCheckoutQuickCheckout extends Controller {
	public function index() {
		...
		if (isset($this->session->data['shipping_method']) &&
			isset($this->session->data['shipping_method']['code']) && !empty($this->session->data['shipping_method']['code']) &&
			isset($this->session->data['shipping_method']['title']) && !empty($this->session->data['shipping_method']['title']) &&
			isset($this->session->data['shipping_method']['cost']) && !empty($this->session->data['shipping_method']['cost']) &&
			isset($this->session->data['shipping_method']['tax_class_id']) && !empty($this->session->data['shipping_method']['tax_class_id']) &&
			isset($this->session->data['shipping_method']['text']) && !empty($this->session->data['shipping_method']['text']) 
		) {
	...
	public function save() {
		...
		$shipping_methods       = isset($this->session->data['shipping_method']) ? $this->session->data['shipping_method'] : '';
		$shipping_code         = isset($this->session->data['shipipng_code']) ? $this->session->data['shipping_code'] : '';
		...
		if ($this->customer->isLogged()) {
			...
			if (!isset($this->session->data['shipping_method'])||!isset($this->session->data['shipping_method'])) {
			...
			if (
				...
				$shipping_country_id     = isset($this->session->data['shipping_address']['country_id']) ? $this->session->data['shipping_address']['country_id'] : 209;
		...
		if (
			...
			$shipping_city           = $this->request->post['shipping_city'];
			$shipping_zone_id        = $this->request->post['shipping_zone'];
			$zone_info               = $this->model_localisation_zone->getZone(isset($this->session->data['shipping_address']['zone_id'])?$this->session->data['shipping_address']['zone_id']:$this->request->post['shipping_zone']);
			$shipping_zone           = $zone_info['name'];
			$country_info               = $this->model_localisation_country->getCountry(isset($this->session->data['shipping_address']['country_id'])?$this->session->data['shipping_address']['country_id']:209);
			$shipping_country        = $country_info['name'];
			$shipping_country_id     = isset($this->session->data['shipping_address']['country_id']) ? $this->session->data['shipping_address']['country_id'] : $this->request->post['country_id'];
			$shipping_telephone      = $this->request->post['shipping_telephone'];
			$shipping_address_format = '';
			...
			$payment_country_id     = $shipping_country_id;
			$payment_zone           = $zone_info['name'];
			$payment_zone_id        = $this->request->post['shipping_zone'];
			$payment_taxid          = '';
			$payment_address_format = $shipping_address_format;
			$payment_custom_field   = $shipping_custom_field;
			$payment_method         = '';
		...
		foreach ($results as $result) {
			if ($this->config->get('shipping_' . $result['code'] . '_status')) {
				...
				$country_id = isset($shipping_country_id) ? $shipping_country_id : 209;
				$zone_id = isset($shipping_zone_id) ? $shipping_zone_id : 3192;
		...
		$order_data['tracking'] = (isset($this->session->data['tracking'])&&!empty($this->session->data['tracking']))?$this->session->data['tracking']:((isset($this->request->post['tracking'])?$this->request->post['tracking']:''));
		...
		// $this->log->white(json_encode($this->cart->getProducts()));

				$this->log->write( json_encode($this->cart->getProducts()) );
		...
		$this->log->write("Add Order #" . json_encode($order_data['products']));
			...
			// if (!isset($shipping_method['error'])) {
				foreach ($shipping_method['quote'] as $quote) {
					...
					if ( 
						...
						($country_id!=209 && !in_array($quote['code'], $shipmeth))
						// ( in_array($quote['code'],$shipmeth) && $country_id!=209) || 
						// ( in_array($quote['code'],$shipmeth) && $country_id==209) 
					) {
