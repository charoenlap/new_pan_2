...
class ControllerCheckoutQuickCheckout2 extends Controller {
	public function index() {
		...
		if (!isset($this->session->data['shipping_method']) || (isset($this->session->data['shipping_method'])&&empty($this->session->data['shipping_method']))) {
			$this->session->data['error'] = 'Not Found Shipping Method - ไม่พบการจัดส่ง';
		}
		...
		$shipping_code_cod = (isset($this->session->data['shipping_method']['code']) && $this->session->data['shipping_method']['code'] == 'pickup.pickup') ? true : false;
	...
	public function payment_view () {
		...
		$this->session->data['payment_method']['code'] = !isset($this->request->post['code']) ? $this->request->post['code'] : 'bank_bbl';
		$code = !isset($this->request->post['code'])||empty($this->request->post['code']) ? 'bank_bbl' : $this->request->post['code'];
		$json = $this->load->controller('extension/payment/'.$code);
	...
	public function confirm() {
		...
		if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address']>0 && $this->request->post['shipping_address']!='newshipping') {
			...
			$shipping_zone_id      = $zone_info['zone_id'];
