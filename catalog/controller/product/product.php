...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			foreach ($this->model_catalog_product->getProductOptions($this->request->get['product_id']) as $option) {
				...
				foreach ($option['product_option_value'] as $option_value) {
					// if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						...
						$product_option_value_data[$option_value['name']] = array(
				...
				ksort($product_option_value_data);
			...
			$data['products_free'] = array();

			$results = $this->model_catalog_product->getProductFree($this->request->get['product_id']);
			...
			$download = $this->model_catalog_product->getDownload($this->request->get['product_id']);

			if (count($download)>0) {
				foreach ($download as $key => $value) {
					$data['downloads'][] = array(
						'name' => $value['mask'],
						'href' => $this->url->link('account/download/download', 'download_id=' . $value['download_id'], true)
					); 
				}
			}
			
			if (isset($this->request->get['type_response']) && $this->request->get['type_response'] == 'ajax') {
				$json = $data;
