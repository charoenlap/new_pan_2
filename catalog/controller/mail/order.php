...
class ControllerMailOrder extends Controller {
	...
	public function alert(&$route, &$args) {
		...
		if ($order_info && !$order_info['order_status_id'] && $order_status_id && in_array('order', (array)$this->config->get('config_mail_alert'))) {	
			...
			// $mailphpmailer->subject = html_entity_decode(sprintf($this->language->get('text_subject'), $this->config->get('config_name'), $order_info['order_id']), ENT_QUOTES, 'UTF-8');
			// $mailphpmailer->html = $this->load->view('mail/order_alert', $data);
			...
			// php mailer send mail
				...
				// $input = array(
				// 	'to'      => 'sportstownonline.div@ba-group.in.th', 
				// 	'subject' => 'บริษัท บางกอกแอธเลติก จำกัด  (สำนักงานใหญ่) - '.$order_info['order_id'], 
				// 	'body'    => $this->load->view('mail/order_alert', $data)
				// );
				// $ch = curl_init();
				// curl_setopt($ch, CURLOPT_URL,"https://www.sportstown-online.com/phpmailer.php");
