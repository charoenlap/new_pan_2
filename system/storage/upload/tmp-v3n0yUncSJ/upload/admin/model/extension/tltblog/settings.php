...
class ModelExtensionTltBlogSettings extends Model {
    ...
    public function upgradeFrom2()
    {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tltblog_url_alias");
        ...
        if ($query->num_rows) {
            ...
            if ($language_id !== null) {
                foreach ($aliases as $alias) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_url_alias SET `language_id` = " . (int)$language_id . ", `query` = '" . $alias['query'] . "', `keyword` = '" . $alias['keyword'] . "'");
                }
            }
        }
    }
    ...
    {
        $query = $this->db->query("SHOW COLUMNS FROM " . DB_PREFIX . "tltblog_url_alias WHERE field = 'language_id'");
