...
class ControllerCommonFooter extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
			if (is_file(DIR_IMAGE . $result['image'])) {
				$data['bottom_banner'][] = array(
					...
					'topic'        => $result['topic'],
					'href'         => $result['link'],
					'image'        => $this->model_tool_image->resize($result['image'], 1200, 300),
				);
			}
		}
		...
		$data['facebook'] = $this->config->get('config_social_facebook_kappathailand');
		$data['google'] = $this->config->get('config_social_google_kappathailand');
